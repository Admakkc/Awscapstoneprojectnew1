...
- We'll set CIDR block as 10.90.0.0/16 for our VPC. You can choose whatever you want. This gives us more than 65000 IP address. As you remember, after we set our CIDR block of our VPC, AWS couldn't expend it, we can just add new CIDR block. 
    ...
    - Go to the subnet section on left hand side and click it. Select create subnet. We are gonna create Subnets one by one to follow directions respectively on below; 
        - Create a public subnet named `[aws-capstone-public-subnet-1a](../Desktop/clarusway-aws-devops-DE-07-tr/AWS/projects/Project-503-Capstone-Project-Blog-Page-App-(Django)-on-AWS-Environment)` under the vpc aws-capstone-VPC in AZ us-east-1a with 10.90.10.0/24
...
Description     : aws-capstone-nat-sg
Inbound Rules
VPC             : aws-capstone-vpc
HTTP(80)    ----> anywhere
HTTPS (443) ----> anywhere
...
TOKEN=$(aws --region=us-east-1 ssm get-parameter --name /adem/capstone/token --with-decryption --query 'Parameter.Value' --output text)
git clone https://$TOKEN@github.com/Admakkc/awscapstoneprojectnew1.git
cd /home/ubuntu/awscapstoneprojectnew1
apt install python3-pip -y
apt-get install python3.10-dev default-libmysqlclient-dev -y
pip3 install -r requirements.txt
cd /home/ubuntu/awscapstoneprojectnew1/src
